# Yuney Boilerplate - stundent end year project
...
Setup a development PostgreSQL with Docker. Copy [.env.example](.env) and rename to `.env` - `cp .env.example .env` - which sets the required environments for PostgreSQL such as `POSTGRES_USER`, `POSTGRES_PASSWORD` and `POSTGRES_DB`. Update the variables as you wish and select a strong password.
...
You need to set the URL to the NestJS GraphQL Api. Open the frontend file who contain env configuration and update `uri`:
